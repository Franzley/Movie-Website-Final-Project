...
export const SearchResults = () => {
  ...
  const onChange = (e) => {
    ...
    fetch(
      `https://api.themoviedb.org/3/search/movie?api_key=55e81c3707b1511daf33d639a483655c&language=en-US&page=1&include_adult=false&query=${e.target.value}`
    )
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
        if (!data.errors) {
          //if no errors, set results to api results
          setResults(data.results);
        } else {
          //if error, set results to empty
          setResults([]);
        }
      });
  };
